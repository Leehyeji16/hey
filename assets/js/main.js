...
const dashboardBottom = () => {
  ...
  const mirrorModeWrap = () => {
    ...
    btnMirrorMode.addEventListener('click', (e) => {
      e.stopPropagation();
    ...
    document.addEventListener('click', (e) => {
      ...
      if (!mainPage.classList.contains('has_mirrormode')) return;

      if (e.target.closest('.btn_mirrormode')) return;

      //  해제
      mainPage.classList.remove('has_mirrormode');
    });
  };
  mirrorModeWrap();
}
